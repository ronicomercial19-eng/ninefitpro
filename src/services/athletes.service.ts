 ...
 * All components should use this service instead of direct Supabase queries
 */
...
export async function getAthleteByUserId(userId: string): Promise<ApiResponse<Athlete>> {
  try {
    ...
    if (!data && !error) {
      // Fallback: check athlete_auth_link
    ...
    if (error || !data) {
      return {
        success: false,
        error: { code: 'NOT_FOUND', message: 'Athlete not found for this user' }
      };
    }
...
export async function getAthleteByEmail(email: string): Promise<ApiResponse<Athlete>> {
  try {
    ...
    if (error || !data) {
      return {
        success: false,
        error: { code: 'NOT_FOUND', message: 'Athlete not found with this email' }
      };
    }
...
export async function activateAthlete(
  ...
  try {
    ...
    if (error) {
      return {
        success: false,
        error: { code: 'ACTIVATION_ERROR', message: error.message }
      };
    }

    // Create/update athlete_auth_link
    await supabase
      .from('athlete_auth_link')
      .upsert({ athlete_id: id, user_id: userId }, { onConflict: 'athlete_id' });
