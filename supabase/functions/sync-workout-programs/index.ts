    ...
    for (const program of base44Programs) {
      try {
        const { data: existing } = await supabase
          .from('programs').select('*').eq('program_name', program.name).single();

        if (existing) {
          const { data: updated, error } = await supabase
            .from('programs').update({ description: program.description || '', updated_at: new Date().toISOString() })
            .eq('id', existing.id).select().single();
          if (error) throw error;
          syncResults.push({ action: 'updated', program: updated });
        } else {
          const { data: created, error } = await supabase
            .from('programs').insert({ program_name: program.name, description: program.description || '' })
            .select().single();
          if (error) throw error;
          syncResults.push({ action: 'created', program: created });
        }
      } catch (syncError) {
        syncResults.push({ action: 'error', program: program.name, error: syncError instanceof Error ? syncError.message : 'Unknown error' });
      }
    }
