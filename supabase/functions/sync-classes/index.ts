      ...
      try {
        ...
        if (existing) {
          const { data: updated, error } = await supabase
            .from('gym_classes').update(classData).eq('id', existing.id).select().single();
          if (error) throw error;
          syncResults.push({ action: 'updated', class: updated });
        } else {
          const { data: created, error } = await supabase
            .from('gym_classes').insert(classData).select().single();
          if (error) throw error;
          syncResults.push({ action: 'created', class: created });
        }
      } catch (syncError) {
        syncResults.push({ action: 'error', class: classItem.name, error: syncError instanceof Error ? syncError.message : 'Unknown error' });
      }
    }
